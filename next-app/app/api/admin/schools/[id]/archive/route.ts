...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await prisma.audit_logs.create({
      data: { user_id: user.id, action: 'SCHOOL_ARCHIVED', module: 'SCHOOLS', entity_type: 'SCHOOL', entity_id: id, school_id: id, result: 'SUCCESS' },
    });

    return NextResponse.json({ success: true, data: { id, archived: true, school: updated, message: 'Établissement archivé avec succès.' } });
  } catch (err: any) {
